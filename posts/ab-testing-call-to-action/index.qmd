...
This confidence interval tells us that, based on our data, we are 95% confident that the true difference in sign-up rates between CTA A and CTA B falls within this range. If the interval does not contain zero, that is initial evidence that there is a real difference between the two CTAs. One advantage of the bootstrap is that it does not require deriving a closed-form formula for the standard error, which can be difficult in more complex settings.
...
In practice, this justifies using normal-based inference methods even when the underlying data are binary.
...
While statistical significance tells us whether the difference is likely to be real, it does not tell us whether the difference is meaningful in practice.

In our example, even if the difference in sign-up rates is statistically significant, a 4 percentage point increase (from 18% to 22%) should be evaluated in terms of business impact. For instance:

- How many additional users does this translate to?
- Does it justify the cost of implementing the new CTA?
- Are there downstream effects (e.g., retention, engagement)?

This highlights the difference between statistical significance and practical significance — both are important in decision-making.
